...
>
> **5. 登入失敗 ≠ 重新申請。**(這是「員工進不去 → 重新申請 → 系統竟放行」的 bug 根因)
> 驗證碼 / 密碼輸錯只能**留在登入流程**:重試、或「重寄驗證碼到原登錄信箱」、或「重設密碼(寄到原信箱)」。
> **絕不可**在登入失敗後彈出「重新申請 / 首次登入」入口 —— 一旦失敗能轉成新申請,等於失敗後另開一條免驗證的入口。
>
> **6. 「重新申請」必須先對 `profiles` 去重,且永遠不自動放行。**
> 收到申請時先 `findByEmail`:
> - email **已建檔**(不論 approved / pending / suspended)→ **拒絕新申請**,提示「此信箱已建檔,請改用登入或聯絡管理員重設」。
>   絕不可再建第二筆、絕不可把既有帳號重新標成 approved、絕不可放行。
> - email **未建檔** → 進「待審核」佇列,**仍需**經理 / GM 核准並指派 `role` 後才可登入。
>
> 任何申請路徑都不得 auto-approve。已核准者若忘記 / 輸錯驗證碼,走的是「重設」而非「重新申請」。
>
> 對應 code(收件端 + 申請端):
> ```js
> // A. SSO / 登入收件
...
> if (profile.status !== 'approved') return showAccessDenied();// 待審核 / 停用 → 不放行
...
>
> // B. 「重新申請」入口(登入失敗不可進到這裡)
> function applyForAccess(email) {
>   const existing = db.profiles.findByEmail(email);
>   if (existing) return reject('此信箱已建檔,請改用登入或聯絡管理員重設'); // 去重,不放行
>   return enqueuePendingApproval(email);               // 一律待審核,不 auto-approve
> }
>
> // C. 登入失敗:只能重試 / 重設,不可導向 applyForAccess()
> function onOtpWrong() { showRetryOrReset(); /* ❌ 不呼叫 applyForAccess */ }
> ```
