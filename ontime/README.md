...
- **公司 email 登入**：與 TimeTree 唯一不同——不用 Apple/Google，一律以 **@teicomposite.com** 公司信箱 + 驗證碼登入，非公司同仁無法進入。
...
| フィード（活動 + 聊天合流） | 動態：活動 + 日曆聊天，未讀數 | ✅ |
| Keep（共享備忘/連結，可釘選） | Keep：備忘 / 連結，可釘選 | ✅ |
| 行程搜尋 | 標題/地點/備註全文搜尋 | ✅ |
| 推播提醒 | 行程前 N 分鐘本機通知（SW） | ✅ |
| Apple / Google 登入 | **公司 email @teicomposite.com 驗證** | 🔁 刻意不同 |

> 你接下來要持續加的功能，照同一套資料層與分頁架構擴充即可。

## 登入（公司 email 認證）

- 輸入公司帳號 → 自動補 `@teicomposite.com` → 寄 6 位數驗證碼 → 登入。
- **原型**：示範同仁帳號 `kai / june / sunny / stanley / kate / keira`，OTP 接受任意 6 碼。
- **正式**：填入 Supabase 金鑰後改用 `SB.auth.signInWithOtp()` / `verifyOtp()`；
  migration 內已加 `auth.users` 觸發器，**非 @teicomposite.com 一律擋下**。
- 從姊妹系統 SSO 翻頁進來（`?from=projflow&u=&email=`）直接認回身份、免再登入。
...
瀏覽器開 `ontime/index.html`（或部署網址）→ 用 `kai`（任意 6 碼）登入。
底部分頁切換；右下 ＋ 新增行程/ Keep；頂部切換器選日曆；🔍 搜尋；「更多」可開推播、安裝、翻頁、登出。
