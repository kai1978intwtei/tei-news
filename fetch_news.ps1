...
function Install-DailyTask {
    try {
        ...
        # 用 wscript.exe + silent_runner.vbs 啟動，完全不會閃出視窗
        ...
        # 立即開始，每 30 分鐘執行一次（每天循環 24 小時不間斷）
        ...
        Write-Host "[setup] 已設定每 30 分鐘自動更新（首次 $($startAt.ToString('HH:mm'))，24 小時不間斷）" -ForegroundColor Green
    } catch {
...
function Publish-ToGitHub {
    ...
    try {
        ...
        $vercelUrl = "https://teisalessys.vercel.app/news"
...
# 同仁請自行加入書籤：https://teisalessys.vercel.app/news （TEi-Salesys 系統首頁在 /）
